...
LABEL org.opencontainers.image.source https://github.com/aknochow/minEE/blob/main/cs9/Containerfile
...
RUN echo 'if [ ${EUID} != 0 ]; then export PS1="\n \[\e[1m\]\[\033[01;37m\]\u\[\033[35m\]@\h \[\033[37m\]\w \[\033[32m\]\$(git_branch)\[\033[00m\]\n $ " ; fi' >> /etc/profile.d/prompt.sh
RUN echo 'if [ ${EUID} == 0 ]; then export PS1="\n \[\e[1m\]\[\033[01;31m\]\u\[\033[35m\]@\h \[\033[31m\]\w \[\033[32m\]\$(git_branch)\[\033[00m\]\n $ " ; fi' >> /etc/profile.d/prompt.sh
RUN echo 'printf "\n\e[01;35m$(cat /etc/redhat-release)\e[00;00m\n"' >> /etc/profile.d/prompt.sh

# Set built on date
RUN echo 'printf "minEE (cs9) (built on '$(date)')\n"' > /etc/profile.d/prompt_built_on.sh
